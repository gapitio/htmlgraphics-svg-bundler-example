...
const linkHandler = (
  ...
  if (!url && customProperties.showcase) {
    url = customProperties.dummyUrl;
  }

  if (url) {
    if (customProperties.autofitpanels) {
...
const sidebarBuildingLinkHandler = () => {
  for (const building of customProperties.buildings) {
    if (building.id !== customProperties.currentBuilding) {
      linkHandler(
        building.url,
...
const sidebarLinkHandler = () => {
  linkHandler(customProperties.mapUrl, "map-click", "map-text");
  linkHandler(customProperties.campusUrl, "campus-click", "campus-text");
...
const dataHallLinkHandler = () => {
  for (const dataHall of customProperties.dataHalls) {
    /*
...
const podLinkHandler = () => {
  for (const pod of customProperties.pods) {
 ...
 */
const setLoadRange = () => {
  const { range, baseUnit } = customProperties.loadBar;

  const { value: minValue, unit: minUnit } = formatFixedSI(
    range[0],
    baseUnit,
    0
  );
  const { value: maxValue, unit: maxUnit } = formatFixedSI(
    range[1],
    baseUnit,
    0
  );

  const minValueFormatted = `${minValue} ${minUnit}`;
  const maxValueFormatted = `${maxValue} ${maxUnit}`;

  htmlNode.getElementById(
    "load-bar-range-low"
  ).textContent = `< ${minValueFormatted}`;
  htmlNode.getElementById(
    "load-bar-range-mid"
  ).textContent = `${minValueFormatted} - ${maxValueFormatted}`;
  htmlNode.getElementById(
    "load-bar-range-high"
  ).textContent = `${maxValueFormatted} <`;
};
